...
import swalign
import math
import Utils.DirectoryUtils as Dirs


# todo write on my own.

# uses a gap penalty of 10
def getScoreSW(aa1, aa2, gap_penalty=-10):
    # set blosum scoring matrix
    scoring = swalign.ScoringMatrix(Dirs.PATH_BLOSUM_45)
...
def getDistanceSW(aa1, aa2, length_dep=False, gap_penalty=-10):
    if length_dep:
        if len(aa1) != len(aa2):
            return 1.0
